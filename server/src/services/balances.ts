...
export function getCustomerBalance(customerId: number): {
  ...
  total_goods_value: number; advance_payment: number; balance_due: number;
} {
  const cust = db.prepare('SELECT opening_balance, advance_payment FROM customers WHERE id = ?').get(customerId) as
    | { opening_balance: number; advance_payment: number }
    | undefined;
  ...
  `).get(customerId) as { total_bill: number; total_cash: number };
