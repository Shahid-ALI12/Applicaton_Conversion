...
authRouter.post('/login', loginLimiter, validateBody(z.object({
  ...
  const row = db.prepare('SELECT id, name, username, password_hash, role, is_active FROM users WHERE username = ?').get(username) as
    | { id: number; name: string; username: string; password_hash: string; role: string; is_active: number }
    | undefined;
...
authRouter.post('/change-password', requireAuth, validateBody(z.object({
  ...
  const row = db.prepare('SELECT password_hash FROM users WHERE id = ?').get(req.user!.userId) as { password_hash: string };
  if (!bcrypt.compareSync(current, row.password_hash)) throw AppError.badRequest('Current password ghalat hai.');
