...
licenseRouter.get('/status', (_req, res) => {
  const support = db.prepare("SELECT value FROM settings WHERE key = 'support_phone'").get() as { value: string | null } | undefined;
...
licenseRouter.post('/activate', activateLimiter, validateBody(z.object({
  ...
  try {
    const status = activateLicense((req.body as { code: string }).code);
    res.json(status);
  } catch (err) {
    if (err instanceof LicenseError) throw AppError.badRequest(err.message);
    throw err;
  }
});
