...
usersRouter.get('/', requireAuth, requireAdmin, (req, res) => {
  ...
  const total = (db.prepare(`SELECT COUNT(*) as c FROM users WHERE ${where}`).get(...params) as { c: number }).c;
...
usersRouter.put('/:id', requireAuth, requireAdmin, validateBody(z.object({
  ...
  if (id === req.user!.userId && (req.body as any).is_active === false) throw AppError.badRequest('Apna account deactivate nahi kar sakte.');
  const sets: string[] = [];
  const vals: (string | number | boolean)[] = [];
  for (const [k, v] of Object.entries(req.body as Record<string, unknown>)) { sets.push(`${k} = ?`); vals.push(v as any); }
  if (sets.length === 0) throw AppError.badRequest('Kuch update nahi hai.');
  vals.push(id);
...
usersRouter.delete('/:id', requireAuth, requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  if (id === req.user!.userId) throw AppError.badRequest('Apna account delete nahi kar sakte.');
