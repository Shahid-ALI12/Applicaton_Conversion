...
export function runSeed(): void {
  ...
  // Cash accounts
  const cashCount = (db.prepare('SELECT COUNT(*) as c FROM cash_accounts').get() as { c: number }).c;
  if (cashCount === 0) {
  ...
  if (prodCount === 0) {
    ...
    for (const [name, rate] of products) insert.run(name, rate);
    logger.info('Seeded products');
  }

  // Settings
  const setCount = (db.prepare('SELECT COUNT(*) as c FROM settings').get() as { c: number }).c;
  if (setCount === 0) {
