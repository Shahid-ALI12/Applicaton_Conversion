...
customersRouter.get('/', requireAuth, (req, res) => {
    ...
    const activeFlag = req.query.active;
    ...
    const rows = db.prepare(`SELECT id, name, type, phone, is_active, opening_balance, advance_payment, deleted_at, created_at FROM customers WHERE ${where} ORDER BY name ASC LIMIT ? OFFSET ?`).all(...params, pageSize, offset);
...
customersRouter.get('/:id', requireAuth, (req, res) => {
    ...
    const customer = db.prepare('SELECT id, name, type, phone, is_active, opening_balance, advance_payment, deleted_at, created_at FROM customers WHERE id = ?').get(id);
    if (!customer)
...
customersRouter.post('/', requireAuth, validateBody(createCustomerSchema), (req, res) => {
    const body = req.body;
    const r = db.prepare('INSERT INTO customers (name, type, phone, is_active, opening_balance, advance_payment) VALUES (?, ?, ?, ?, ?, ?)').run(body.name, body.type, body.phone ?? null, body.is_active ? 1 : 0, round2(body.opening_balance), round2(body.advance_payment));
...
customersRouter.put('/:id', requireAuth, validateBody(updateCustomerSchema), (req, res) => {
    ...
    for (const [key, value] of Object.entries(parsed)) {
        if (value !== undefined) {
            cols.push(key);
            if (key === 'opening_balance' || key === 'advance_payment')
                vals.push(round2(Number(value)));
