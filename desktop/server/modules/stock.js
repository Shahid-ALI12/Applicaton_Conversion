...
stockRouter.get('/', requireAuth, (req, res) => {
    const { search } = parsePage(req.query);
    let sql = `
...
stockRouter.post('/', requireAuth, validateBody(z.object({
    ...
    const { product_id, location_id, stock_quantity } = req.body;
    db.prepare(`
...
stockRouter.put('/:id', requireAuth, validateBody(z.object({
    ...
    if (body.stock_quantity !== undefined) {
        ...
        vals.push(body.stock_quantity);
    }
    if (body.last_bag_weight_kg !== undefined) {
        sets.push('last_bag_weight_kg = ?');
        vals.push(body.last_bag_weight_kg);
    }
    if (sets.length === 0)
