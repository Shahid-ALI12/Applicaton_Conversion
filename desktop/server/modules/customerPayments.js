...
customerPaymentsRouter.get('/', requireAuth, (req, res) => {
    ...
    const total = db.prepare(`SELECT COUNT(*) as c FROM customer_payments cp JOIN customers c ON c.id=cp.customer_id WHERE ${where}`).get(...params).c;
...
customerPaymentsRouter.post('/', requireAuth, validateBody(z.object({
    ...
    const result = db.transaction(() => {
        const cust = db.prepare('SELECT opening_balance, advance_payment FROM customers WHERE id = ?').get(body.customer_id);
        ...
        const r = db.prepare('INSERT INTO customer_payments (customer_id, payment_date, amount, applied_to_opening, applied_to_advance, opening_balance_before, opening_balance_after, advance_before, advance_after, notes, entered_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(body.customer_id, body.payment_date, round2(body.amount), round2(appliedOpening), round2(appliedAdvance), openingBefore, newOpening, advanceBefore, newAdvance, body.notes ?? null, body.entered_by ?? null);
        // Cash in
        const cashAccount = db.prepare("SELECT id FROM cash_accounts WHERE name = 'Cash In Hand'").get();
        if (cashAccount) {
