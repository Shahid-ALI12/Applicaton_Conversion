...
labourExtrasRouter.post('/labour-daily-wages', requireAuth, validateBody(createWageSchema), (req, res) => {
    ...
    const r = db.prepare(stmtText).run(body.labour_id, body.wage_date, round2(body.amount), body.notes ?? null, body.entered_by ?? null);
    res.status(201).json({ id: Number(r.lastInsertRowid) });
});
// ─────────────────────────────────────────────────────────────────────────────
...
//# sourceMappingURL=labourExtras.js.map