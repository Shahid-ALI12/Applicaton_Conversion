...
salesRouter.get('/', requireAuth, (req, res) => {
    ...
    const total = db.prepare(`SELECT COUNT(*) as c FROM sales s JOIN customers c ON c.id=s.customer_id JOIN products p ON p.id=s.product_id WHERE ${where}`).get(...params).c;
...
salesRouter.post('/', requireAuth, validateBody(z.object({
    ...
    const transaction = db.transaction(() => {
        // Allocate group ID
        const counter = db.prepare("SELECT value FROM counters WHERE key = 'sale_group'").get();
