...
laboursRouter.get('/', requireAuth, (req, res) => {
    ...
    const params = [];
    if (search) {
        where += ' AND (l.name LIKE ? OR l.role LIKE ?)';
        params.push(`%${search}%`, `%${search}%`);
    }
    const total = db.prepare(`SELECT COUNT(*) as c FROM labours l WHERE ${where}`).get(...params).c;
...
//# sourceMappingURL=labours.js.map