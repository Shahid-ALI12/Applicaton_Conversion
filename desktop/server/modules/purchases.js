...
purchasesRouter.get('/', requireAuth, (req, res) => {
    ...
    if (search) {
        where += ' AND (pr.name LIKE ? OR s.name LIKE ?)';
        params.push(`%${search}%`, `%${search}%`);
    }
    const total = db.prepare(`SELECT COUNT(*) as c FROM purchases p JOIN products pr ON pr.id=p.product_id LEFT JOIN suppliers s ON s.id=p.supplier_id WHERE ${where}`).get(...params).c;
