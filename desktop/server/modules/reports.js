...
reportsRouter.get('/reconciliation', requireAuth, (req, res) => {
    ...
    const from = req.query.from ?? pktToday();
  ...
  `).all(from, to);
    ...
    const total_billed = sales.reduce((sum, s) => sum + (s.quantity || 0) * (s.rate_per_bag || 0) + (s.rickshaw_fare || 0), 0);
...
reportsRouter.get('/reconciliation/details', requireAuth, (req, res) => {
    ...
    const pageSize = Math.max(1, parseInt(req.query.pageSize || '50', 10) || 50);
    const customerName = req.query.customer_name?.trim() || '';
    const descriptionSearch = req.query.description?.trim() || '';
    // Resolve customer IDs by name substring (case-insensitive)
    let custIds = null;
    if (customerName) {
        const matched = db.prepare(`SELECT id FROM customers WHERE LOWER(name) LIKE ?`).all(`%${customerName.toLowerCase()}%`);
    ...
    try {
        switch (type) {
            case 'bags-sold': {
                if (customerName && custIds && custIds.length === 0)
                    return res.json({ rows: [], total: 0, page, pageSize, totalPages: 1, label: 'Total Bags Sold' });
                ...
                if (custIds) {
                    sql += ` AND s.customer_id IN (${custIds.map(() => '?').join(',')})`;
                    params.push(...custIds);
                }
                sql += ` ORDER BY s.id DESC`;
                const rows = db.prepare(sql).all(...params).map((s) => ({
            ...
            case 'total-billed': {
                if (customerName && custIds && custIds.length === 0)
                    return res.json({ rows: [], total: 0, page, pageSize, totalPages: 1, label: 'Total Billed' });
            ...
            case 'cash-received': {
                if (customerName && custIds && custIds.length === 0)
                    return res.json({ rows: [], total: 0, page, pageSize, totalPages: 1, label: 'Cash Actually Received' });
            ...
            case 'credit-customers': {
                if (customerName && custIds && custIds.length === 0)
                    return res.json({ rows: [], total: 0, page, pageSize, totalPages: 1, label: 'From Credit Customers' });
            ...
            case 'cash-customers': {
                if (customerName && custIds && custIds.length === 0)
                    return res.json({ rows: [], total: 0, page, pageSize, totalPages: 1, label: 'From Cash Customers' });
                let sql = `SELECT s.id, s.sale_date, s.quantity, s.rate_per_bag, s.rickshaw_fare, s.cash_received, c.name as customer_name, c.type as customer_type, p.name as product_name
                   FROM sales s LEFT JOIN customers c ON c.id=s.customer_id LEFT JOIN products p ON p.id=s.product_id
            ...
            case 'expenses': {
                ...
                const rows = db.prepare(sql).all(...params).map((e) => ({
